...
TEST(MemoryManger, AllocateMultipleLine)
{
    const auto frame1 = mgr.Allocate(BitmapMemoryManager::kBitsPerMapLine + 3);
    const auto frame2 = mgr.Allocate(1);

    CHECK_EQUAL(0, frame1.value.ID());
    CHECK_TRUE(BitmapMemoryManger::kBitsPerMapLine + 3 <= frame2.value.ID());
}

TEST(MemoryManger, AllocateNoEnoughMemory)
{
