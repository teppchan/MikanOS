...
![UTF8を表示できるようになった](img/2021-09-03-22-36-00.png)

UTF8のファイルを表示できるようになった。
...
![日本語を表示できるようになった](img/2021-09-04-22-29-00.png)

FreeTypeの力を借りて日本語が表示できるようになった。すごい。
...
![ファイルにリダイレクトできるようになった。](img/2021-09-05-21-58-00.png)

コマンドの出力結果をファイルに出力できるようになった。
...
![](img/2021-09-08-21-49-00.png)

`$?`でエラーコードを表示できるようになった。

`osbook_day29b`を写した。

![](img/2021-09-08-22-54-00.png)

ちゃんと写したつもりだったけど、パイプを使ってコマンドを実行すると止まってしまった。

バグがみつからないから、明日改めて見直す。
...
![](img/2021-09-09-21-53-00.png)

バグが見つかった。しばらく前からあったみたい。
...
![](img/2021-09-10-21-24-00.png)

`sort`が使えるようになった。

`osbook_day29d`を写した。

![](img/2021-09-10-21-25-00.png)
...
![](img/2021-09-11-20-31-00.png)

`apps`をつけなくてもアプリケーションを実行できるようになった。

コマンドをルートディレクトリで探して、なかったら`apps`を探すという、2段構えになっている。

`osbook_day30b`を写した。

![](img/2021-09-11-20-50-00.png)

`more`でページ送りできるようになった。

`osbook_day30c`を写した。

![](img/2021-09-11-21-05-00.png)

`cat`が標準入力を受けられるようになった。

## 2021/09/12 （132日目）

`osbook_day30d`を写した。

![](img/2021-09-12-21-22-00.png)

閉じるボタンでウィンドウを閉じられるようになった。


`osbook_day30e`を写した。

![](img/2021-09-12-22-05-00.png)

テキストビュア`tview`が使えるようになった。
...
![](img/2021-09-13-20-55-00.png)

グラフィックビュア`gview`が使えるようになった。
絵が見えるようになるとOSっぽさが増す。

これで、一通り写経し終わった。結局133日もかかってしまった。
もう一度本を読み直して、理解を深めよう。
