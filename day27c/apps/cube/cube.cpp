...
void DrawObj(uint64_t layer_id)
{
    ...
    for (;;)
    {
        ...
        const auto v1 = vert[kSurface[sur][1]];
        const auto v2 = vert[kSurface[sur][2]];
        const auto e0x = v1.x - v0.x;
        const auto e0y = v1.y - v0.y;
        const auto e1x = v2.x - v1.x;
        const auto e1y = v2.y - v1.y;
