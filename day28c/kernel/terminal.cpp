...
void Terminal::ExecuteLine()
{
    ...
    else if (strcmp(command, "ls") == 0)
    {
        ...
        else
        {
            ...
            if (dir == nullptr)
            {
                PrintToFD(*files_[1], "No such file or directory: %s\n", first_arg);
            }
    ...
    else if (command[0] != 0)
    {
        ...
        if (!file_entry)
        {
            PrintToFD(*files_[1], "no such command: %s\n", command);
        }
        else if (file_entry->attr != fat::Attribute::kDirectory && post_slash)
        {
            char name[13];
        ...
        {
            PrintToFD(*files_[1], "failed to exec file: %s\n", err.Name());
        }
    }
...
Error Terminal::ExecuteFile(fat::DirectoryEntry &file_entry, char *command, char *first_arg)
{
    ...
    int ret = CallApp(argc.value, argv,
                      3 << 3 | 3,
                      app_load.entry,
